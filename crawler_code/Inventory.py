...
class Inventory(object):
    ...
    def to_dict(self):
        ...
        return {"name": self.name, "IPC_list": self.IPC_list, "children": children}
...
def get_info(driver: WebDriver, output_path="./result.txt") -> Inventory:
    ...
    def cal_ipc_no(inv: Inventory):
        cnt = len(inv.IPC_list)
        for child in inv.children:
            cnt += cal_ipc_no(child)
        return cnt

    print(cal_ipc_no(inventory))
        ...
        for item in IPC_info:
            ...
            for no_text in remain:
                if no_text.find("-") == -1:
                    ...
                    no_front_remain = no_front_remain * 10 if no_front_remain < 100 else no_front_remain
                    # 后
