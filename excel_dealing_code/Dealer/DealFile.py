...
from Dealer.DealIPC import deal_ipc
from Reader.Reader_Factory import get_excel_reader
from environment import green_cnt_log_file
from utils import MY_DEBUG, LOG
...
def get_need_col(file_name):
    reader = get_excel_reader(file_name)
    try:
        title = reader.get_titles()
        key_to_index = get_key_index(title)

        result = []
        cnt = 0
        total_no = 0
        for row_data in reader.travel_row():
            flag = False
            result_data = {}
            for key in need_col:
                ...
                if key in key_to_index:
                    ...
                    data_value = row_data[index]
                    if key == "IPC":
                        ipc_info, green_no = deal_ipc(data_value)
                        result_data.update(ipc_info)
                        total_no += green_no
                    elif key == "专利类型" and data_value is not None and data_value.strip() == "外观设计":
                        flag = True
                        break
                    else:
                        result_data[key] = data_value
                else:
        ...
        print(total_no)
        LOG("{},green_no:{},total_no:{}".format(file_name, total_no, reader.get_row_number() - 1),
            green_cnt_log_file)
...
#             result.append(result_data)
#         print(total_no)
#         LOG("{},green_no:{},total_no:{}".format(file, total_no, sht.nrows - 1), "../output/cnt_log.txt")
#         return result, total_no
#     finally:
...
#             result.append(result_data)
#         print(total_no)
#         LOG("{},green_no:{},total_no:{}".format(file, total_no, sht.max_row - 1), "../output/cnt_log.txt")
#         return result, total_no
#     finally:
#         wb.close()
